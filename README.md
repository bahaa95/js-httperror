...
Install node-httperror with npm.

```bash
  npm install node-httperror
```
 ...
 * at new HttpError (user:\Programing\Wep\node-httperror\test.js:56:32)
...
```typescript
import createHttpError, { Options } from 'node-httperror';
...
```typescript
import createHttpError, { Hydrate } from 'node-httperror';
...
```typescript
import createHttpError, { statuses } from 'node-httperror';
